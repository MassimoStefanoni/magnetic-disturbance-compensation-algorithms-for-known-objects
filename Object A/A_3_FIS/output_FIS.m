% function [out_FIS]=output_NET_fun();
clear all
close all
clc
...
%fisx = readfis('ANFIS_Bx_negl_sys_triang_1_3');  % max abs error  -2.311 uT

% x component
fisx = readfis('FIS_Bx_negl_sys_3_gauss_1_3');   % max abs error   3.17 uT
n_xy=size(out_Hx_);
for nx=1:1:n_xy(1,1) 
    for ny=1:1:n_xy(1,2)
...
% y component
fisy = readfis('FIS_By_negl_sys_3_gauss_1_3');   % 
n_xy=size(out_Hy_);
for nx=1:1:n_xy(1,1) 
    for ny=1:1:n_xy(1,2)
...
% z component
fisz = readfis('FIS_Bz_negl_sys_3_gauss_1_3');   %   
n_xy=size(out_Hz_);
for nx=1:1:n_xy(1,1) 
    for ny=1:1:n_xy(1,2)
...
FIS_xyz_MAE_bef=(sum(sum(abs(out_Hx_(:,:))))/(41*41-15*15)+sum(sum(abs(out_Hy_(:,:))))/(41*41-15*15)+sum(sum(abs(out_Hz_(:,:))))/(41*41-15*15))/3;
FIS_xyz_MAE=(sum(sum(abs(out_e_Bx(:,:))))/(41*41-15*15)+sum(sum(abs(out_e_By(:,:))))/(41*41-15*15)+sum(sum(abs(out_e_Bz(:,:))))/(41*41-15*15))/3;

[M1,I1]=max(abs(out_Hx_)); % Provide the maximum in each column and put it in M1. In I1 the row where there is the maximum.
[M,I]=max(M1);
FIS_x_max_abs_err_bef=abs(out_Hx_(I1(1,I),I));
FIS_x_max_abs_err_elem_n_bef=[I1(1,I),I];
FIS_x_MAE_bef=sum(sum(abs(out_Hx_(:,:))))/(41*41-15*15);

[M1,I1]=max(abs(out_e_Bx)); % Provide the maximum in each column and put it in M1. In I1 the row where there is the maximum.
[M,I]=max(M1);
FIS_x_max_abs_err=abs(out_e_Bx(I1(1,I),I));
FIS_x_max_abs_err_elem_n=[I1(1,I),I];
FIS_x_MAE=sum(sum(abs(out_e_Bx(:,:))))/(41*41-15*15);

[M1,I1]=max(abs(out_Hy_)); % Provide the maximum in each column and put it in M1. In I1 the row where there is the maximum.
[M,I]=max(M1);
FIS_y_max_abs_err_bef=abs(out_Hy_(I1(1,I),I));
FIS_y_max_abs_err_elem_n_bef=[I1(1,I),I];
FIS_y_MAE_bef=sum(sum(abs(out_Hy_(:,:))))/(41*41-15*15);

[M1,I1]=max(abs(out_e_By)); % Provide the maximum in each column and put it in M1. In I1 the row where there is the maximum.
[M,I]=max(M1);
FIS_y_max_abs_err=abs(out_e_By(I1(1,I),I));
FIS_y_max_abs_err_elem_n=[I1(1,I),I];
FIS_y_MAE=sum(sum(abs(out_e_By(:,:))))/(41*41-15*15);

[M1,I1]=max(abs(out_Hz_)); % Provide the maximum in each column and put it in M1. In I1 the row where there is the maximum.
[M,I]=max(M1);
FIS_z_max_abs_err_bef=abs(out_Hz_(I1(1,I),I));
FIS_z_max_abs_err_elem_n_bef=[I1(1,I),I];
FIS_z_MAE_bef=sum(sum(abs(out_Hz_(:,:))))/(41*41-15*15);

[M1,I1]=max(abs(out_e_Bz)); % Provide the maximum in each column and put it in M1. In I1 the row where there is the maximum.
[M,I]=max(M1);
FIS_z_max_abs_err=abs(out_e_Bz(I1(1,I),I));
FIS_z_max_abs_err_elem_n=[I1(1,I),I];
FIS_z_MAE=sum(sum(abs(out_e_Bz(:,:))))/(41*41-15*15);

out_FIS=[ FIS_x_max_abs_err_bef; FIS_x_MAE_bef; FIS_x_max_abs_err; FIS_x_MAE;  
             FIS_y_max_abs_err_bef; FIS_y_MAE_bef; FIS_y_max_abs_err; FIS_y_MAE;
             FIS_z_max_abs_err_bef; FIS_z_MAE_bef; FIS_z_max_abs_err; FIS_z_MAE;
             FIS_xyz_MAE_bef; FIS_xyz_MAE;];
         
csvwrite("Results_FIS.txt",out_FIS);
