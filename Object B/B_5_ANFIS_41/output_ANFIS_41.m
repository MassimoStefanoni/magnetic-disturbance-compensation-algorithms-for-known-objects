...
% x component
fisx = readfis('ANFIS_Bx_negl_sys_triang_41');   % max abs error  -2.311 uT
...
for nx=1:1:n_xy(1,1) 
    for ny=1:1:n_xy(1,2)
        ...
        out_e_Bx_fig(nx,ny)=out_Hx_(nx,ny)-out_fis_Bx(nx,ny); 
...
% y component
fisy = readfis('ANFIS_By_negl_sys_triang_41');   % 
n_xy=size(out_Hy_);
for nx=1:1:n_xy(1,1) 
    for ny=1:1:n_xy(1,2)
        out_fis_By(nx,ny)=evalfis(fisy,[nx ny]); 
        out_e_By(nx,ny)=out_Hy_(nx,ny)-out_fis_By(nx,ny); 
        out_e_By_fig(nx,ny)=out_Hy_(nx,ny)-out_fis_By(nx,ny); 
...
% z component
fisz = readfis('ANFIS_Bz_negl_sys_triang_41');   %   
n_xy=size(out_Hz_);
for nx=1:1:n_xy(1,1) 
    for ny=1:1:n_xy(1,2)
        out_fis_Bz(nx,ny)=evalfis(fisz,[nx ny]); 
        out_e_Bz(nx,ny)=out_Hz_(nx,ny)-out_fis_Bz(nx,ny); 
        out_e_Bz_fig(nx,ny)=out_Hz_(nx,ny)-out_fis_Bz(nx,ny); 
...
out_ANFIS_41=[ ANFIS_x_max_abs_err_bef; ANFIS_x_MAE_bef; ANFIS_x_max_abs_err; ANFIS_x_MAE;  
 ...
 csvwrite("Results_out_ANFIS_41.txt",out_ANFIS_41);
...
name_figure="eBx_heat_ANFIS_41.png";
...
zlabel(my_z_label,'Fontsize', my_myfontsize_label,'fontweight','bold');
%     xlim(my_xlim); %     zlim(my_zlim); %     xticks(my_xticks); %     yticks(my_yticks); %     zticks(my_zticks);
name_figure="eBy_heat_ANFIS_41.png";
...
zlabel(my_z_label,'Fontsize', my_myfontsize_label,'fontweight','bold');
%     xlim(my_xlim); %     zlim(my_zlim); %     xticks(my_xticks); %     yticks(my_yticks); %     zticks(my_zticks);
name_figure="eBz_heat_ANFIS_41.png";
